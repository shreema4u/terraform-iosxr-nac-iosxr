resource "iosxr_logging" "logging" {
  ...
  buffered_logging_buffer_size = try(local.device_config[each.value.name].logging.buffered_logging_buffer_size, local.defaults.iosxr.devices.configuration.logging.buffered_logging_buffer_size, null)
