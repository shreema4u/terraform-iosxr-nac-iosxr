  ...
  device_pces = [
    for device in local.devices : {
      ...
      api_users = try(length(local.device_config[device.name].pce.api_users) == 0, true) ? null : [for api_user in local.device_config[device.name].pce.api_users : {
        user_name          = try(api_user.user_name, local.defaults.iosxr.configuration.pce.api_users.user_name, null)
        password_encrypted = try(api_user.password_encrypted, local.defaults.iosxr.configuration.pce.api_users.password_encrypted, null)
        }
      ]
      state_sync_ipv4s = try(length(local.device_config[device.name].pce.state_sync_ipv4s) == 0, true) ? null : [for state_sync_ipv4 in local.device_config[device.name].pce.state_sync_ipv4s : {
        address = try(state_sync_ipv4.address, local.defaults.iosxr.configuration.pce.state_sync_ipv4s.address, null)
        }
      ]
    }
